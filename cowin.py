...
APIKEY = "1754468723:AAHFqk7AX7OAkSiw7V53OIhuU9GmLhlz7nE"
CHATID = "-409554185"


def vaccine_notifers(pcode, age):
    today = date.today()
    ...
    for d in data["sessions"]:
        if d["min_age_limit"] == 45 and d['available_capacity']>0:
    ...
    for d in data["sessions"]:
        if d["min_age_limit"] == 18 and d['available_capacity']>0:
            centre_detail = "Centre {0}: ".format(cnt) + "\nCentre Adrress: " + d['name'] + ", " + d[
                "address"] + "\nVaccine: " + d['vaccine'] + "\nAvailable Capacity dose 1: " + str(
                d["available_capacity_dose1"]) + "\nAvailable Capacity dose 2: " + str(
                d["available_capacity_dose2"]) + '\n'
    ...
    # base_url = 'https://api.telegram.org/bot' + APIKEY + '/sendMessage?chat_id=' + CHATID + '&text={0}'.format(
    #     messageFor45)
